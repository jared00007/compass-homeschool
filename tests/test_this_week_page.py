"""Mission Control's Board and Plan-a-lesson views.

The old day-by-day "Plan next week" batch planner was removed entirely
(reported: "remove that 100%"); planning is now the per-subject
"Plan a lesson" view, which generates a whole topic as a multi-day series.
The Board view -- every subject's stories laid out on one week -- is
unchanged and covered below alongside the new Plan view.
"""
...
# --- the Plan-a-lesson view: pick a subject, generate the whole series ----------


def _open_plan_view(monkeypatch, db_path):
    ...
    _switch_mc_view(at, "plan")
    return at


def test_the_old_day_by_day_week_planner_is_gone(monkeypatch, tmp_path):
    """The removed batch planner: none of its widgets survive anywhere in
    Mission Control (reported: "remove that 100%")."""
    ...
    assert not any(k.startswith("regen_week_") for k in keys)
    assert not any(k.startswith("regen_day_") for k in keys)
...
def test_plan_view_offers_a_button_per_subject_and_defaults_to_math(monkeypatch, tmp_path):
    ...
    assert any((b.key or "") == "math_gen_series" for b in at.button)


def test_switching_subject_shows_that_subjects_panel(monkeypatch, tmp_path):
    db_path = tmp_path / "plan.db"
    db = Database(db_path)
    db.ensure_default_student()
    db.close()
...
def test_generating_a_series_from_mission_control(monkeypatch, tmp_path):
    """The whole point: pick a subject in Mission Control and one click writes
    the topic as an ordered, dateless series -- no day-by-day planning."""
    from unittest.mock import patch

    db_path = tmp_path / "plan.db"
    db = Database(db_path)
    s = db.ensure_default_student()
    db.close()

    at = _open_plan_view(monkeypatch, db_path)
    plan = [{"title": "Day A", "focus": "a"}, {"title": "Day B", "focus": "b"}]
    payload = {
        "title": "Generated", "activities": [{"minutes": 30}],
        "estimated_minutes": 30,
        "subject_credits": [{"subject": "math", "minutes": 30, "justification": ""}],
    }
    with patch("compass.agents.series.plan_lesson_series", return_value=plan), patch(
        "compass.agents.framework.generate_lesson", side_effect=lambda **k: dict(payload)
    ):
        [b for b in at.button if (b.key or "") == "math_gen_series"][0].click().run(timeout=60)
    assert not at.exception, [e.message for e in at.exception]

    db = Database(db_path)
    lessons = db.list_lessons(s["id"], agent="math", limit=10)
    db.close()
